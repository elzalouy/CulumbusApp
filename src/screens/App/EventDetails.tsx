...
import { View, Text, Dimensions, StyleSheet, Image, TouchableOpacity, ScrollView, ImageBackground } from 'react-native'
...
//   }
  ...
  return (
   ...
   <View style={styles.container}>
        ...
        <FastImage source={{uri:event.cover_image_url}} style={styles.img} resizeMode={FastImage.resizeMode.cover}
               onLoad={props.onload}/>

        <View style={{flexDirection:'row', marginTop:w*0.03}}>
        {/* <Icon  name={'location'} size={normalize(15)} color={'#203152'} /> */}
        <Text style={styles.location}>{event.city.name} / {event.city.country.name}</Text>
        </View>
        <Text style={styles.description}>{event.about}</Text>
        <Text style={[styles.description,{marginTop:w*0.05}]}>{event.description}</Text>

        {event.latitude?
        <View style={{width:w*0.9,paddingVertical:w*0.03,marginTop:w*0.1}}>
       <MapView
 ...
 {/* <MapView.Marker
    ...
    /> */}
     ...
     <Icon1 name={'directions'} color="#2767AD" style={{position:'absolute',top:w*0.04,left:w*0.01}}  size={normalize(40)} onPress={()=>handleGetDirections()}  />

     </View>
     :null
     }

        <TouchableOpacity style={styles.loginButton} onPress={()=>props.navigation.navigate('Reservation',{event:event})}>
