...
function CityOfTheDay(props) {
  ...
  const [city, setCity] = useState({name:'',cover_image_url:'',country:{name:''}})
  const [options, setOptions] = useState(['Activites','Adrenaline','Hotels','Flights','Shopping'])
...
if (props.events.length !== events.length){
  ...
  return (
    <View style={styles.container}>
        ...
        <Text style={styles.txt}>{city.name}, {city.country.name}</Text>
        ...
        <SkeletonContent
                        ...
                        highlightColor="white"
                          layout={[
                            { key: '4',width:w*0.9,
                            height:w*0.5, borderRadius:10 },
                           ]}
                        />
                        :null}
        <FastImage source={{uri:city.cover_image_url}} style={styles.img} resizeMode={FastImage.resizeMode.cover}
               onLoad={()=>setmainLoaded(false)}/>
        </View>
        <ScrollView style={styles.optionsContainer} horizontal showsHorizontalScrollIndicator={false}>
        {options.map((item,index)=>
       
        <TouchableOpacity style={selectedOption!==index?styles.optionCard:styles.selectedOptionCard} onPress={()=>setSelectedOption(index)} activeOpacity={1.0}>
          <Text style={selectedOption!==index?styles.optionText:styles.selectedOptionText}>{item}</Text>
        </TouchableOpacity>
