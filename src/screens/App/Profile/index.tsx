...
import { View, Text, Dimensions, TouchableOpacity, ScrollView, Platform, PermissionsAndroid } from 'react-native'
...
function Profile(props) {
  ...
  const [visitedCountries, setVisitedCountries] = useState({"type":"FeatureCollection","features":[]});
  const [wishlistedCountries, setWishlistedCountries] = useState({"type":"FeatureCollection","features":[]});
...
if (JSON.stringify(props.user) !== JSON.stringify(user)){
  ...
  useEffect(() => {
    ...
    let countries=['egypt','italy','switzerland', 'germany','austria','turkey']
    let features=[]
    ...
    let wishlistedFeatures=[]
     for(let i=0;i< myPlace.features.length;i++){
       if(countries.includes(myPlace.features[i].properties.name.toLowerCase())){
        features.push(myPlace.features[i])
       }
       if(wishlistedCountries.includes(myPlace.features[i].properties.name.toLowerCase())){
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{alignItems:'center'}}>
        ...
        <View style={{flex:1}}>
        <TouchableOpacity style={styles.editContainer} onPress={()=>props.navigation.navigate('EditProfile')}>
        <Text style={styles.editText}>Edit</Text>
        <Icon name='edit' style={{marginLeft:5,marginBottom:5}} size={normalize(10)} color='white'/>
        </TouchableOpacity>
        </View>
        </View>
        ...
        <MapView
            ...
            minZoomLevel={0}
            maxZoomLevel={0}
            style={{ width:w*0.94,height:w*0.6,borderRadius:15,marginTop:h*0.02 }}
        ...
        <TouchableOpacity style={styles.logoutButton} activeOpacity={1.0}onPress={()=>props.navigation.navigate('Logout')}>
