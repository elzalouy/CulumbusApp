  ...
  if (props.loading !== loading){
    ...
    function reset(){
        if(password===confirmPassword){
          ...
          props.resetPassword(data,props.navigation)
        }
        ...
        return (
            ...
            <ImageBackground source={{uri: 'img2'}} style={styles.container}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.txt2}>Please enter the new password in the form bellow:</Text>
                </View>
               ...
               <ActivityIndicator style={{marginTop:w*0.5}} size="large" color="#203152" />
                ...
                <View style={styles.form} >
                    ...
                    <View>
                    <TextInput style={styles.input} placeholder="Enter a new password" placeholderTextColor="rgba(32, 49, 82, 0.2)"
                    onChangeText={text => setConfirmPassword(text)}
                    value={confirmPassword}
                    secureTextEntry={showPassword?false:true} 
                    />
                    <TouchableOpacity style={{position:'absolute',right:0}} onPress={()=>setShowPassword(!showPassword)} activeOpacity={1.0}>
                    <Icon1  name={showPassword?'eye':'eye-with-line'} size={normalize(23)} color={'#D0D0D0'} />
                    </TouchableOpacity>
                    </View>
                    <View >
                    <TextInput style={styles.input} placeholder="Confirm new password" placeholderTextColor="rgba(32, 49, 82, 0.2)"
                    onChangeText={text => setPassword(text)} 
                                       
                    value={password}      
                    secureTextEntry={showConfirmPassword?false:true} 
                    />
                    <TouchableOpacity style={{position:'absolute',right:0}} onPress={()=>setShowConfirmPassword(!showConfirmPassword)} activeOpacity={1.0}>
                    <Icon1  name={showConfirmPassword?'eye':'eye-with-line'}  size={normalize(23)} color={'#D0D0D0'} />
                    </TouchableOpacity>
                    </View>

                    </View>
                    <TouchableOpacity style={styles.loginButton} onPress={()=>reset()}>
                    <Text style={styles.txt4}>UPDATE PASSWORD</Text>
                    </TouchableOpacity>
                </View>
}
